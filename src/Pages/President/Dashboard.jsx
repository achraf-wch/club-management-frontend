...
const PresidentDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch Pending Requests — real data from GET /api/clubs/{clubId}/requests/stats
      if (clubData?.id) {
        const reqRes = await fetch(`${API_BASE_URL}/api/clubs/${clubData.id}/requests/stats`, { credentials: 'include' });
        if (reqRes.ok) {
          const reqData = await reqRes.json();
          setPendingCount(reqData.pending_requests ?? 0);
        }
      }
    } catch (error) {
  ...
  const actions = [
    { label: 'Assigner Billets', Icon: IconTicket, path: '/President/AssignTickets', accent: '#22c55e' },
    { label: 'Gérer les Membres', Icon: IconSettings, path: '/President/Members', accent: '#3b82f6' },
    { label: 'Nouvel Événement', Icon: IconPlus, path: '/President/AddEvent', accent: '#c0392b' },
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 overflow-x-hidden ${dm ? 'bg-[#0a0a0f] text-white' : 'bg-[#f8fafc] text-[#1a2c5b]'}`}>
      ...
      <div className="max-w-7xl mx-auto px-8 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            {/* Hero Welcome Card — now includes Security section */}
            <div className={`relative rounded-[3rem] p-10 overflow-hidden border transition-all ${dm ? 'bg-white/5 border-white/10' : 'bg-[#1a2c5b] text-white shadow-2xl'}`}>
              ...
              {/* ── Security & Account — embedded in hero ── */}
          ...
          {/* Sidebar (4 cols) — Actions Rapides redesign */}
          <div className="lg:col-span-4">
            <div className={`relative rounded-[3rem] p-8 border overflow-hidden h-full flex flex-col ${dm ? 'bg-white/5 border-white/10' : 'bg-white border-gray-100 shadow-xl'}`}>
              ...
              <div className="relative z-10 mt-6 flex gap-1">
                {actions.map((a, i) => (
                  <div key={i} className="h-0.5 flex-1 rounded-full" style={{ background: `${a.accent}60` }}></div>
                ))}
              </div>
            </div>
          </div>
      ...
      <style>{`
        ...
        .action-btn { animation: fadeUp 0.4s ease both; }
