...
const AdminLayout = () => {
  ...
  const resolveAvatar = (u) => {
    if (!u) return '';
    return u.avatar_url || u.profile_photo_url || u.avatar || u.picture || '';
  };

  useEffect(() => {
    const url = resolveAvatar(user);
    setAvatarSrc(url ? `${url}?t=${Date.now()}` : '');
    setImgError(false);
  }, [user?.avatar_url, user?.avatar, user?.picture, user?.profile_photo_url]);
  ...
  const navGroups = [
    ...
    },
    {
      ...
      label: 'Clubs',
      items: [
        { label: 'Créer un Club',  path: '/admin/addClub' },
        { label: 'Gérer les Clubs', path: '/admin/deleteClub' },
      ]
    },
    {
      key: 'president',
      label: 'Ajouter un Président',
      single: true,
...
>
  ...
  <div className="leading-tight">
    ...
    <p className="text-red-400 text-[10px] font-medium tracking-wide">Administrateur</p>
  </div>
</div>
