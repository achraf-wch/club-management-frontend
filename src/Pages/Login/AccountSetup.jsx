...
const TwoFactorSetup = ({ user, API_BASE_URL }) => {
  ...
  const inputCls = 'w-full px-4 py-3.5 bg-black/60 border-2 border-red-900/30 text-white rounded-xl focus:outline-none focus:border-red-500 transition-all duration-300 placeholder-gray-600 text-center text-2xl font-mono tracking-widest';
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div className="mb-8">
        ...
        <p className="text-gray-600 text-sm">Protégez votre compte avec Google Authenticator</p>
      </div>

      {success && (
        <div className="bg-green-900/20 border border-green-700/40 text-green-300 px-5 py-4 rounded-xl flex items-center gap-3">
      ...
      {error && (
        <div className="bg-red-900/20 border border-red-700/40 text-red-300 px-5 py-4 rounded-xl flex items-center gap-3">
      ...
      <div className={`px-5 py-4 rounded-xl border flex items-center gap-3 ${is2FAEnabled ? 'bg-green-900/15 border-green-800/30' : 'bg-red-900/15 border-red-800/30'}`}>
        <div className={`w-3 h-3 rounded-full flex-shrink-0 ${is2FAEnabled ? 'bg-green-400' : 'bg-red-500'}`} />
        <div>
          <p className={`text-sm font-semibold ${is2FAEnabled ? 'text-green-300' : 'text-red-300'}`}>
      ...
      {!is2FAEnabled && status !== 'setup' && (
        <button onClick={handleSetup} disabled={isLoading}
          className="group relative w-full py-4 bg-black border-2 border-red-800/50 text-red-400 font-bold rounded-xl hover:bg-red-950/30 hover:border-red-600/60 hover:text-red-300 transition-all duration-300 disabled:opacity-50 overflow-hidden flex items-center justify-center gap-3">
      ...
      {status === 'setup' && (
        <div className="space-y-5">
          <div className="bg-black/40 border border-red-900/20 rounded-xl p-6">
            ...
            <div className="p-3 bg-black/60 rounded-lg border border-red-900/20">
              <p className="text-xs text-gray-600 mb-1">Ou entrez ce code manuellement :</p>
              <p className="text-red-400 font-mono text-sm tracking-widest break-all">{secret}</p>
            </div>
          </div>

          <div className="bg-black/40 border border-red-900/20 rounded-xl p-6">
            <p className="text-white font-semibold mb-1">Étape 2 — Confirmer</p>
            <p className="text-gray-500 text-sm mb-4">Entrez le code à 6 chiffres affiché dans l'application</p>
            <input
              type="text"
              maxLength={6}
              value={code}
          ...
          <button onClick={handleEnable} disabled={isLoading || code.length !== 6}
            className="w-full py-4 bg-black border-2 border-red-800/50 text-red-400 font-bold rounded-xl hover:bg-red-950/30 transition-all duration-300 disabled:opacity-50">
      ...
      {is2FAEnabled && (
        <div className="space-y-4">
          <div className="bg-black/40 border border-red-900/20 rounded-xl p-6">
            ...
            <p className="text-gray-500 text-sm mb-4">Entrez votre code actuel pour désactiver</p>
            <input
              type="text"
              maxLength={6}
              value={code}
...
const AccountSetup = () => {
  ...
  const inputCls = 'w-full px-4 py-3.5 bg-black/60 border-2 border-red-900/30 text-white rounded-xl focus:outline-none focus:border-red-500 focus:bg-black/80 transition-all duration-300 placeholder-gray-600';
          ...
          <div className="bg-[#0d0d18] border-r border-red-900/20 p-8 flex flex-col">
            <div className="text-center mb-8">
              <div className="relative inline-block mb-4">
                <div className="w-28 h-28 rounded-full overflow-hidden border-4 border-red-900/40 shadow-xl ring-4 ring-black">
                  {avatarPreview ? (
                ...
                <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 bg-red-600 px-3 py-0.5 rounded-full shadow-lg">
              ...
              <p className="text-red-900/70 text-xs font-mono bg-red-900/10 border border-red-900/20 px-3 py-1 rounded-full inline-block">{user?.member_code}</p>
            </div>
            ...
            <nav className="space-y-2 flex-1">
              {tabs.map((tab) => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-semibold text-sm transition-all duration-300 text-left ${activeTab === tab.id ? 'bg-red-600/20 border border-red-600/40 text-red-400 shadow-lg' : 'text-gray-500 hover:bg-red-900/10 hover:text-gray-300 border border-transparent'}`}>
                  <span className={activeTab === tab.id ? 'text-red-400' : 'text-gray-600'}>{renderIcon(tab.icon)}</span>
                  {tab.label}
                  {activeTab === tab.id && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-red-500"></span>}
                </button>
              ))}
            </nav>
            ...
            {activeTab === 'google' && (
              <div className="space-y-6 animate-fadeIn">
                <div className="mb-8">
                  ...
                  <p className="text-gray-600 text-sm">{googleStatus.is_linked ? `Connecté avec ${googleStatus.google_email}` : 'Connectez-vous facilement avec Google'}</p>
                </div>
