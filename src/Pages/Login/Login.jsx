  ...
  return (
    <div style={{ userSelect: 'none', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      <div ref={circleRef} style={{
        ...
        overflow: 'hidden',
        position: 'relative',
        animation: 'floatYeti 3.2s ease-in-out infinite',
        flexShrink: 0,
      }}>
        <svg width="140" height="155" viewBox="0 0 140 155" xmlns="http://www.w3.org/2000/svg">
          ...
          {[20,29,38].map((x,i)=>(
            <ellipse key={i} cx={x} cy={118-i*2} rx="6" ry="10" fill="#d4eaf8" transform={`rotate(${-20+i*5} ${x} ${118-i*2})`}/>
          ))}
          {[120,111,102].map((x,i)=>(
            <ellipse key={i} cx={x} cy={118-i*2} rx="6" ry="10" fill="#d4eaf8" transform={`rotate(${20-i*5} ${x} ${118-i*2})`}/>
          ))}
          ...
          <g transform={headTransform} style={{ transition: 'transform 0.12s ease-out' }}>
            ...
            {[40,50,58,66,74,82,90,100].map((x,i)=>(
              <ellipse key={i} cx={x} cy={50+(i===0||i===7?6:i===1||i===6?3:0)} rx="7" ry="12"
                fill="#e2f0fa" transform={`rotate(${-30+i*8} ${x} 50)`}/>
            ))}
            ...
            {!isCovering && (
              <>
                <path d={isWatching ? "M 44 78 Q 54 74 64 78" : "M 44 76 Q 54 72 64 76"}
                  stroke="#5a9ab5" strokeWidth="3" fill="none" strokeLinecap="round" style={{ transition: 'd 0.3s ease' }} />
                <path d={isWatching ? "M 76 78 Q 86 74 96 78" : "M 76 76 Q 86 72 96 76"}
                  stroke="#5a9ab5" strokeWidth="3" fill="none" strokeLinecap="round" style={{ transition: 'd 0.3s ease' }} />
              </>
            )}
            ...
            {!isCovering && (
              <path d="M 58 114 Q 70 124 82 114" stroke="#5a9ab5" strokeWidth="3" fill="none" strokeLinecap="round" />
            )}
            {isCovering && (
              <path d="M 60 118 Q 70 114 80 118" stroke="#5a9ab5" strokeWidth="3" fill="none" strokeLinecap="round" />
            )}
          ...
          <style>{`
            @keyframes riseLeft {
              from { transform: translateY(50px) translateX(-20px); opacity: 0; }
      ...
      <div className="mt-3 px-4 py-1.5 rounded-2xl text-xs font-semibold text-center relative transition-all duration-400"
        style={{
          background: 'linear-gradient(135deg, #011C40, #023859)',
          border: '1px solid rgba(84,172,191,0.35)',
          color: '#A7EBF2',
          minWidth: '165px',
          boxShadow: '0 4px 20px rgba(84,172,191,0.12)',
        }}
      >
...
const Login = () => {
  ...
  // ── Dark mode: sync with Navbar toggle via custom event ──
  ...
  const inputStyle = (field) => ({
    ...
    border: errors[field]
      ...
      : focusedField === field
      ? '1px solid rgba(84,172,191,0.9)'
      : '1px solid rgba(84,172,191,0.2)',
    color:      '#e2e8f0',
    boxShadow:  focusedField === field ? '0 0 0 3px rgba(84,172,191,0.1), 0 0 20px rgba(84,172,191,0.1)' : 'none',
  ...
  const yetiProps = {
    focusedField,
    mousePos,
    typing: loginData.email.length > 0,
  };

  // ── dark = noir pur / light = bleu clair ──
            ...
            <div className="text-center space-y-2 mt-2">
              <h1 className="text-4xl font-bold" style={{
                ...
                textShadow: darkMode
                  ...
                  : '0 0 16px rgba(185,28,28,0.35), 2px 2px 8px rgba(185,28,28,0.15)',
              }}>
                Bienvenue
              </h1>
              <p className="text-base" style={{ color: 'rgba(239,68,68,0.75)' }}>
                sur <span style={{
                  color: '#dc2626',
                  fontWeight: 600,
                  textShadow: darkMode ? '0 0 12px rgba(220,38,38,0.7)' : 'none',
                }}>CluVersity</span>
              </p>
              <p className="text-sm" style={{
                color: 'rgba(220,38,38,0.55)',
                textShadow: darkMode ? '0 0 8px rgba(220,38,38,0.4)' : 'none',
              }}>
                EST Fès — Clubs & Événements
              </p>
            </div>
              ...
              {['#b91c1c','#dc2626','#ef4444'].map((c,i) => (
                <div key={i} className="w-2 h-2 rounded-full animate-pulse"
                  style={{
                    ...
                    boxShadow: darkMode ? `0 0 6px ${c}` : 'none',
                  }} />
              ))}
            </div>
          </div>
          ...
          <div className="w-full max-w-md mx-auto">
            ...
            <div ref={formRef}>
              <div className="relative p-8 rounded-3xl overflow-hidden"
                style={{
                  background: 'linear-gradient(160deg, rgba(1,28,64,0.92) 0%, rgba(10,25,41,0.97) 100%)',
                  border:     '1px solid rgba(84,172,191,0.18)',
                  boxShadow:  '0 30px 70px rgba(0,0,0,0.55), inset 0 1px 0 rgba(84,172,191,0.08)',
                }}
              >
                ...
                <div className="text-center mb-7">
                  <h2 className="text-3xl font-bold mb-1.5 tracking-wide" style={{
                    color: '#ef4444',
                    textShadow: darkMode
                ...
                <div className="mb-4">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ref={emailRef}
                      type="email" name="email" value={loginData.email}
                ...
                <div className="mb-7">
                  ...
                  <div className="relative">
                    ...
                    <input
                      type="password" name="password" value={loginData.password}
                      onChange={handleChange} onKeyPress={handleKeyPress}
                ...
                {/* ── Bouton Se connecter — ROUGE avec animations ── */}
                <style>{`
                  ...
                  @keyframes btnGlowPulse {
                    0%, 100% { box-shadow: 0 8px 30px rgba(220,38,38,0.35), 0 0 0 0 rgba(220,38,38,0); }
                    50%      { box-shadow: 0 8px 40px rgba(220,38,38,0.6), 0 0 20px 4px rgba(220,38,38,0.25); }
                  }
                `}</style>
