...
const BureauxAssignTickets = () => {
  ...
  const fetchMyClub = async () => {
    try {
      // Bureaux uses my-club-info (works for both president and board)
      ...
      <div className="max-w-5xl mx-auto px-4 space-y-6">
        ...
        <div>
          <h1 className={`text-4xl font-bold ${dm ? 'text-red-400' : 'text-gray-900'}`}>
            Assigner des <span className="text-red-500">Billets</span>
          </h1>
          {club && (
            <p className={`mt-1 text-sm ${dm ? 'text-gray-500' : 'text-gray-500'}`}>
              Envoyez des billets aux membres de <span className={`font-semibold ${dm ? 'text-red-400' : 'text-red-500'}`}>{club.name}</span>
            </p>
          )}
        </div>
                ...
                <button
                  ...
                  className={`w-full py-4 rounded-xl font-bold text-sm transition-all duration-300 disabled:opacity-40 disabled:cursor-not-allowed hover:scale-[1.02]
                    ${dm
                      ...
                      : 'bg-gradient-to-r from-[#0a3d62] to-[#0c5087] text-white shadow-lg shadow-blue-200/50'}`}
                >
                  {loading ? (
