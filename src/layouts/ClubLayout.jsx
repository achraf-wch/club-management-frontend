...
const ClubLayout = () => {
  ...
  // ── the only logic difference between the two old layouts ──
  ...
  useEffect(() => {
    const url = user?.profile_photo_url || user?.avatar_url || user?.avatar || '';
  ...
  // ── merged nav — president sees everything, bureaux sees less ──
  const navGroups = [
    {
      key: 'dashboard',
      label: 'Dashboard',
    ...
    },
    {
      ...
      label: 'Membres',
      items: [
        { label: 'Ajouter un Membre', path: '/club/members/add' },
        { label: 'Liste des Membres', path: '/club/members' },
        // president only ↓
        ...(isPresident
          ? [{ label: 'Voir les Demandes', path: '/club/demandes' }]
          : []),
      ],
    },
    {
      ...
      label: 'Événements',
      items: [
        { label: 'Créer un Événement', path: '/club/events/create' },
        { label: 'Assigner les Tickets', path: '/club/tickets/assign' },
        { label: 'Scanner les Tickets', path: '/club/events/scan' },
        // president only ↓
        ...(isPresident
          ? [
              { label: 'Gérer les Événements', path: '/club/events/manage' },
            ]
          : []),
      ],
    },
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            >
              ...
              <div className="leading-tight">
                ...
                {/* ── only this line changes between roles ── */}
                <p className="text-red-400 text-[10px] font-medium tracking-wide">
                  {roleLabel}
                </p>
              </div>
            </div>
              ...
              return (
                <div key={group.key} className="relative h-full flex items-center">
                  <button
                    ...
                    className={`relative flex items-center gap-1.5 px-5 h-full text-xs font-semibold transition-all
